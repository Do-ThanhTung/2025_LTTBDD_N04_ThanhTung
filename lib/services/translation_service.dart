...
class TranslationService {
  ...
  static const _prefsKeyTargetLang =
  ...
  Future<void> loadConfig() async {
    try {
      ...
      _targetLang =
          prefs.getString(_prefsKeyTargetLang) ?? 'vi';
    } catch (_) {}
  }
  ...
  Future<String?> translateLocal(String text,
    ...
    try {
      final res =
          await compute(_translateLocalIsolate, args);
      return res;
    } catch (_) {
      return null;
    }
  ...
  Future<void> mergeMappings(String jsonString,
      {required String to}) async {
    try {
      final Map<String, dynamic> m =
      ...
      if (to == 'vi') {
        ...
        await prefs.setString(
            _prefsKeyEnVi, json.encode(merged));
      } else {
        m.forEach((k, v) {
          if (v is String) _viToEn[_normalize(k)] = v;
        });
        ...
        merged.addAll(m);
        await prefs.setString(
            _prefsKeyViEn, json.encode(merged));
      }
    } catch (_) {
      // ignore parse errors
    }
  ...
  Future<Map<String, String>> getEnglishMap() async {
    ...
    try {
      final s = await rootBundle
          .loadString('assets/data/local_en_vi.json');
      final Map<String, dynamic> m = json.decode(s);
      m.forEach((k, v) {
