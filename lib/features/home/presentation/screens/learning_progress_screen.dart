  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      if (isLoggedIn) {
        ...
        savedWords = prefs.getStringList(
                '${provider}_saved_words') ??
            [];
        readStories = prefs.getStringList(
                '${provider}_read_stories') ??
            [];
        todayWords =
            prefs.getInt('${provider}_today_words') ??
                0;
        currentStreak = prefs.getInt(
                '${provider}_current_streak') ??
            0;
      } else {
