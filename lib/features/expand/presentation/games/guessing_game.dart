...
class _GuessingGameState extends State<GuessingGame> {
  ...
  String _tr(String key, Map<String, String> params) =>
      AppLocalizations.tr(context, key,
          params: params);
  ...
  Future<void> _loadAndStartGame() async {
    ...
    // 5. Dịch tất cả definition sang tiếng Việt
    ...
    setState(() {
      ...
      if (_availableWords.isNotEmpty) {
        ...
        if (wordLength > 2 &&
            _revealedPositions.length <
                initialReveal) {
          final random = Random();
          final middlePositions = List.generate(
              wordLength - 2, (i) => i + 1);
  ...
  void _useHint() {
    ...
    setState(() {
      ...
      final hiddenPositions = List.generate(
              wordLength, (i) => i)
          .where(
      ...
      int toAdd =
          targetReveal - _revealedPositions.length;
      for (int i = 0;
          i < min(toAdd, hiddenPositions.length);
          i++) {
    ...
    if (!isLoggedIn) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text(_t('flashcard_login_required')),
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      _hintsUsed == 1
                          ? _t(
                              'guessing_hints_used_one')
                          : _hintsUsed == 2
                              ? _t(
                                  'guessing_hints_used_two')
                              : _t(
                                  'guessing_hints_used_three'),
                      style: TextStyle(
