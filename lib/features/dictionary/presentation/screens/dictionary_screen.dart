...
import 'package:education/core/models/api.dart';
import 'package:education/core/models/response_model.dart';
import 'package:education/core/services/search_history_service.dart';
import 'package:education/core/services/vocabulary_service.dart';
import 'package:education/features/auth/presentation/screens/login_screen.dart';
...
class _DictionaryScreenState
  ...
  Future<void> _checkLoginStatus() async {
    ...
    if (mounted) {
      setState(() {
        // Clear saved words if logged out
        if (!loggedIn) {
          _savedWords.clear();
        }
        isLoggedIn = loggedIn;
  ...
  Future<void> _handleSaveWordClick(
      String word) async {
    ...
    final provider =
        prefs.getString('login_provider') ?? 'default';
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(savedWords.contains(word)
            ? 'Đã lưu từ'
            : 'Đã bỏ lưu từ'),
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(
                AppLocalizations.t(context, 'login')),
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText:
              _t(context, 'dictionary_search_hint'),
          hintStyle: TextStyle(
        ...
        if (isLoggedIn) {
          ...
          final provider =
              prefs.getString('login_provider') ??
                  'default';
          final searchedWordsKey =
              '${provider}_searched_words';
          final searchCountKey =
              '${provider}_search_count';
          final savedWordsKey =
              '${provider}_saved_words';
          ...
          if (!searchedWords
            ...
            await prefs.setStringList(
                searchedWordsKey, searchedWords);
            await prefs.setInt(
                searchCountKey, searchedWords.length);
          }
  ...
  Widget _buildResponseWidget() {
    ...
    // Check login status every time build
    _checkLoginStatus();
